...
Sub emeidiInsertDocs()
    ...
    GoTo Canceled:
        End If
         ...
         With doc.Shapes
            With .AddOLEObject(Filename:=Path, LinkToFile:=False, DisplayAsIcon:=True, IconFileName:=Icon(0), IconIndex:=Icon(1), IconLabel:=IconLabel, Anchor:=Application.Selection.Range)
                'Might cause problems when already InlineShape
    ...
    Next
     ...
     ' THE OLD WAY:
     'ActiveDocument.Shapes.AddOLEObject Filename:=Path, LinkToFile:=False, DisplayAsIcon:=True, IconLabel:=IconLabel, Anchor:=Application.Selection.Range
